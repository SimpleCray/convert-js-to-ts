...
export default function MegaMenuDesktopHorizon({ data, ...other }) {
	return (
		<Stack direction='row' spacing={ITEM_SPACING} {...other}>
			{data.map((parent) => (
				<MegaMenuItem key={'MegaMenuDesktopHorizon-key' + parent.title} parent={parent} />
			))}
		</Stack>
	);
}
				...
				{open && (
					...
					>
						<Masonry columns={3} spacing={2}>
							{children.map((list) => (
								<Stack key={'MegaMenuItem-key' + list.subheader} spacing={1.25} sx={{ mb: 2.5 }}>
									...
									{list.items.map((link) => (
										<Link
											key={'MegaMenuItemLink-key' + link.title}
						...
						{!!more && !!tags && !!products && (
							<Stack spacing={3}>
								<Link component={RouterLink} href={more?.path} sx={{ typography: 'body2', display: 'inline-flex', fontSize: 13 }}>
									{more?.title}
								</Link>
