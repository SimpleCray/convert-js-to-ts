		...
		<Controller
			...
			render={({ field, fieldState: { error } }) => (
				<FormControl component='fieldset'>
					...
					>
						{options.map((option) => (
							<FormControlLabel key={'RHF-checkbox-key-' + option.value} control={<Checkbox checked={field.value.includes(option.value)} onChange={() => field.onChange(getSelected(field.value, option.value))} />} label={option.label} {...other} />
						))}
					</FormGroup>
